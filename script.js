...
function showGuessAndCorrectLocation(userGuess, correctLatLng) {
    ...
    const nextButton = modal.querySelector('.next-button'); // Get the next button from the modal
    ...
    modalMap.fitBounds(extendedBounds, {
        padding: [50, 50],
        duration: 0.5,
        animate: true
    });
    ...
    modalTitle.textContent = currentQuestionInfo.name;
    modalDistance.textContent = `Distance: ${document.getElementById('distance').textContent}`;
    modalScore.textContent = `Score: ${document.getElementById('score').textContent}`;
    modalLocationInfo.innerHTML = `
        <img src="${currentQuestionInfo.image}" alt="${currentQuestionInfo.name}">
        <p>${currentQuestionInfo.info}</p>
    `;

    nextButton.style.display = 'block'; // Show the next button in the modal
